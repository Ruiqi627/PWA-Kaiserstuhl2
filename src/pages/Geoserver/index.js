...
class Geoserver extends React.Component {
	...
	Download = () => {
		...
		root.att('xmlns:webdienste',"http://localhost:8090/geoserver/webdienste");
		...
		for(var i = 0; i < window.pointsarrayall.length; i++) {
			...
			var item = insert.ele('webdienst:testlayer');
			...
			var accuracy = item.ele('Accuracy', window.pointsarrayall[i][0].Accuracy)
			var color = item.ele('Color', window.pointsarrayall[i][0].Color)
